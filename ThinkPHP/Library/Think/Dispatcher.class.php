...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') ||  !Route::check()) ){   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            if( C('MODULE_REWRITE') && in_array( $_GET[$varController] , C('REWRITE_CONTROLLER')) ){    //页面重定向
                $url = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
                $rewrite_to = M(C('REWRITE_TABLE'))->where("url='{$url}'")->getField('rewrite_to');
                if( !empty($rewrite_to) ){
                    redirect($rewrite_to,0,'',301);
                }
            }
            // 获取操作
