...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(isset($route)) {
            ...
            $url    =   (__APP__ == '/index.php'?'':__APP__).'/'.($module?$module.MODULE_PATHINFO_DEPR:'').implode($depr,array_reverse($var));
        }
        if($urlCase){
            $url    =   strtolower($url);
        }
        if(!empty($vars)) { // 添加参数
            foreach ($vars as $var => $val){
                if('' !== trim($val))   $url .= $depr . (C('URL_PARAMS_BIND_TYPE')?'':$var . $depr) . urlencode($val);
            }                
        }
        if($suffix) {
 ...
 * @return void
 */
function redirect($url, $time=0, $msg='',$code=302) {
    //多行URL地址支持
